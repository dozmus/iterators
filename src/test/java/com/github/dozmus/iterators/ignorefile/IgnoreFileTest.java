package com.github.dozmus.iterators.ignorefile;
...
public class IgnoreFileTest {
    ...
    private static final String IGNORE_FILE_DIR = "src/test/resources/com/github/dozmus/iterators/ignorefile/";

    private static void assertIgnored(Path path, String ruleText) {
        Rule rule = IgnoreFile.parse(BASE_DIR, ruleText);
        Assert.assertTrue(IgnoreFile.ignored(path, rule));
    }

    private static void assertNotIgnored(Path path, String ruleText) {
        Rule rule = IgnoreFile.parse(BASE_DIR, ruleText);
        Assert.assertFalse(IgnoreFile.ignored(path, rule));
    }

    private void assertIgnored(List<Rule> rules, Path path) {
        Assert.assertTrue(IgnoreFile.ignored(path, rules));
    }

    private void assertNotIgnored(List<Rule> rules, Path path) {
        Assert.assertFalse(IgnoreFile.ignored(path, rules));
    }
    ...
    public void testIgnored() throws IOException {
        Path path = Paths.get(IGNORE_FILE_DIR, ".customignore");
        List<Rule> rules = IgnoreFile.read(IGNORE_FILE_DIR, path);
